...
namespace emp {
	...
	inline block privacy_free_garble(block LA0, block A1, block LB0, block B1, block delta, block* table, uint64_t idx,
									 const AES_KEY* key) {
	...
	public:
		...
		block and_gate(const block& a, const block& b) {
			block table[1];
			block res = privacy_free_garble(a, a ^ delta, b, b ^ delta, delta, table, gid++, &prp.aes);
