 ...
 */
	...
	inline std::ostream& operator<<(std::ostream& out, const block& blk) {
		...
		out << std::setw(16) << std::setfill('0') << data[1] << std::setw(16) << std::setfill('0') << data[0];
...
#define OUT(x, y) out[(y) *nrows / 8 + (x) / 8]
	...
	sse_trans(uint8_t* out, uint8_t const* inp, uint64_t nrows, uint64_t ncols) {
		...
		for (rr = 0; rr <= nrows - 16; rr += 16) {
			for (cc = 0; cc < ncols; cc += 8) {
				vec = _mm_set_epi8(INP(rr + 15, cc), INP(rr + 14, cc), INP(rr + 13, cc), INP(rr + 12, cc),
								   ...
								   INP(rr + 7, cc), INP(rr + 6, cc), INP(rr + 5, cc), INP(rr + 4, cc), INP(rr + 3, cc),
								   INP(rr + 2, cc), INP(rr + 1, cc), INP(rr + 0, cc));
		...
		if ((ncols % 8 == 0 && ncols % 16 != 0) || (nrows % 8 == 0 && nrows % 16 != 0)) {
			...
			for (cc = 0; cc <= ncols - 16; cc += 16) {
				vec = _mm_set_epi16(*(uint16_t const*) &INP(rr + 7, cc), *(uint16_t const*) &INP(rr + 6, cc),
