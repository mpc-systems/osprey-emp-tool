...
namespace emp {
	...
	template <typename T>
	int aes_128_ctr(const __m128i key, const __m128i iv,
		...
		if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_ctr(), NULL, (const unsigned char*) (&key),
	...
	// There is some internal state used during encryption, so don't try to use the same object in 2 threads
	// simultaneously: just make multiple objects.
	...
	public:
		...
		int aes_128_ctr(const block key[], const block iv[],
			...
			if (length < 129) {
				...
				for (uint64_t i = 0; (128 * i) < length; ++i) {
					answer = this->aes_128_ctr(nullptr, iv, (input == nullptr) ? nullptr : &(input[128 * i]),
		...
		int aes_128_ctr(const block key[],
			...
			for (uint64_t i = 0; (128 * i) < length; ++i) {
				...
				answer = this->aes_128_ctr(nullptr, &(emp::Integer(128, &counter, party).bits[0].bit),
