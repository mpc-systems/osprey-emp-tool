...
namespace emp {
	...
	// There is some internal state used during each hash, so don't try to use the same object in 2 threads
			...
			for (i = 0; i < input_count; ++i) {
				for (j = 0; j < input_sizes[i]; ++j) {
					blocks[keccak_address_translator(index)] =
						CircuitExecution::circ_exec->xor_gate(blocks[keccak_address_translator(index)], inputs[i][j]);
					++index;
					if (index == 1088) {
		...
		void sha3_256(block output[], // we'll write 256 blocks to here as the output hash.
			...
			for (i = 0; i < input_count; ++i) {
				for (j = 0; j < inputs[i].bits.size(); ++j) {
					blocks[keccak_address_translator(index)] = CircuitExecution::circ_exec->xor_gate(
						blocks[keccak_address_translator(index)], inputs[i].bits[j].bit);
					++index;
					if (index == 1088) {
						keccak_f->compute(blocks, blocks);
						index = 0;
