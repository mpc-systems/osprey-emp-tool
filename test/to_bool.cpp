...
int main() {
	...
	for (uint64_t i = 0; i < len; ++i) {
		...
		if (output[i].y != (i + 1) << 50) {
			std::cerr << "in to_bool, output[" << i << "].y was " << output[i].y << " when it should be "
					  << ((i + 1) << 50) << "\n";
			return -1;
		}
		if (output[i].z != ((uint32_t) i) << 20) {
			std::cerr << "in to_bool, output[" << i << "].z was " << output[i].z << " when it should be "
					  << (((uint32_t) i) << 20) << "\n";
			return -1;
		}
		if (output[i].t != (uint8_t) i) {
			std::cerr << "in to_bool, output[" << i << "].t  was " << output[i].t << " when it should be "
					  << ((uint8_t) i) << "\n";
			return -1;
		}
	}
